...
if ! [ -f "dist/ffmpeg" ]; then
    ...
    # Or download the universal binary from: https://evermeet.cx/ffmpeg/getrelease/ffmpeg/zip
...
echo " => Creating dmg..."
mkdir -p dist/dmg
mv dist/OnTheSpot.app dist/dmg/OnTheSpot-universal.app
...
hdiutil create -srcfolder dist/dmg -format UDZO -o dist/OnTheSpot-universal.dmg
...
echo " => Done! .dmg available in 'dist/OnTheSpot-universal.dmg'."
